...
func (e BundleEntityID) EntityID() (EntityID, error) {
	id, err := util.JSONMarshal(e)
	return EntityID(id), err
}

func BundleID(id EntityID) (BundleEntityID, error) {
...
func ListEntities(ctx context.Context, catsrc *v1alpha1.CatalogSource, timeout time.Duration) ([]*Entity, error) {
	conn, err := grpc.ConnectWithTimeout(ctx, catsrc.Address(), timeout)
	if conn != nil {
		defer conn.Close()
	}
	if err != nil {
	...
	for {
		...
		entity, err := entityFromBundle(fmt.Sprintf("%s/%s", catsrc.Namespace, catsrc.Name), bundle)
		if err != nil {
...
func entityFromBundle(catsrcName string, bundle *catalogsourceapi.Bundle) (*Entity, error) {
	...
	return NewEntity(id, props), nil
}

func idFromBundle(catsrcID string, bundle *catalogsourceapi.Bundle) (EntityID, error) {
...
func deduplicate(entities []*Entity) ([]*Entity, error) {
	entityMap := map[EntityID]*Entity{}
	resultSet := []*Entity{}
	errs := map[EntityID][]error{}
